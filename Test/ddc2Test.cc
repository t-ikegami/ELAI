...
void ddc0( MPI_Comm intra, MPI_Comm inter )
{
  ...
  {
    int nnz = All.action().nnz();
    float *a = All.action().val();
    for ( int k = 0; k < nnz; ++k ) a[ k ] = k;
  }
  A.reflectIn( All, subj );
  F.reflect( f, subj );
  F_sync();
}
