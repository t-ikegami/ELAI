...
int main( int argc, char **argv )
{
  ...
  for ( int i = 0; i < mysize; ++i ) palette.push_back( i );
  ...
  if ( myrank == 1 ) cout << "sub A" << myrank << ": " << A.action();
  MPI_Barrier( MPI_COMM_WORLD );

  {
    Vector& x = f.ran();
    //for ( int i = 0; i < x.m(); ++i ) x( i ) = 0. + myrank;
    for ( int i = 0; i < x.m(); ++i ) x( i ) = static_cast< float >( i ) * ( myrank + 1 );
  }

  if ( myrank == 1 ) cout << "Before Sync sub" << myrank << ": " << f.ran();
  coherent( f.ran().val() );
  if ( myrank == 1 ) cout << "Before Sync sub" << myrank << ": " << f.ran();

  Function F( all );
  Sync F_sync( F, MPI_COMM_WORLD );

  F.reflect( f, subj );
  F_sync();
  if ( myrank == 0 ) cout << "all: " << F.ran();
