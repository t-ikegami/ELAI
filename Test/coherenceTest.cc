...
int main( int argc, char **argv )
{
  ...
  if ( myrank == 0 )
  {
    ...
    s1.join( Element( 4, PSI, myrank ), Element( 3, PSI, pair ) );
    s1.join( Element( 5, PSI, myrank ), Element( 2, PSI, pair ) );

    Function f( s1 );
    Coherence coherent( f, MPI_COMM_WORLD );

    for ( int i = 0; i < n; ++i ) f( Element( i, PSI, myrank ) ) = 2 * i;
    coherent( f.ran().val() );
    cout << f.ran();
  }
  else if ( myrank == 1 )
  {
    ...
    s1.join( Element( 0, PSI, myrank ), Element( 3, PSI, pair ) );
    s1.join( Element( 1, PSI, myrank ), Element( 2, PSI, pair ) );
    s1.join( Element( 2, PSI, myrank ) );
    s1.join( Element( 3, PSI, myrank ) );
    s1.join( Element( 4, PSI, myrank ) );
    s1.join( Element( 5, PSI, myrank ) );

    Function f( s1 );
    Coherence coherent( f, MPI_COMM_WORLD );

    for ( int i = 0; i < n; ++i ) f( Element( i, PSI, myrank ) ) = 2 * i + 1;
    coherent( f.ran().val() );
    cout << f.ran();
  }
  MPI_Finalize();
