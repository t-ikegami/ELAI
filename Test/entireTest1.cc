...
typedef elai::entire_function< Element, Neighbour, float > Function_Dist;
typedef elai::entire_operator< Element, Neighbour, float > Operator_Dist;
...
int main( int argc, char **argv )
{
  ...
  if ( myrank == 0 )
  {
    ...
    Operator A2( port );
    Function f2( port );
    Operator_Dist A_dist;
    Function_Dist f_dist;
    A_dist.join( A1 );
    A_dist.join( A2 );
    f_dist.join( f1 );
    f_dist.join( f2 );
