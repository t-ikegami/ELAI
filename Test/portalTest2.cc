...
int main( int argc, char **argv )
{
  ...
  if ( myrank == 0 )
  {
    ...
    u0.join( Element( 0, PSI, myrank ) );
    u0.join( Element( 1, PSI, myrank ) );

    Neighbour u1( Element( 1, PSI, myrank ) );
    u1.join( Element( 0, PSI, myrank ) );
    u1.join( Element( 1, PSI, myrank ) );
    u1.join( Element( 2, PSI, myrank ) );

    Neighbour u2( Element( 2, PSI, myrank ) );
    u2.join( Element( 1, PSI, myrank ) );
    u2.join( Element( 2, PSI, myrank ) );
    u2.join( Element( 3, PSI, myrank ) );

    Neighbour u3( Element( 3, PSI, myrank ) );
    u3.join( Element( 2, PSI, myrank ) );
    u3.join( Element( 3, PSI, myrank ) );
    u3.join( Element( 4, PSI, myrank ) );

    Neighbour u4( Element( 4, PSI, myrank ) );
    u4.join( Element( 3, PSI, myrank ) );
    u4.join( Element( 4, PSI, myrank ) );

    Family T1( s1 );
    T1.join( u0 ).join( u1 ).join( u2 ).join( u3 ).join( u4 );
